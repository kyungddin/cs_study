...
- Makefile 문법
    ...
    - Dependencies: 빌드 대상이 의존하는 Target이나 파일 목록. 여기에 나열된 대상들을 "먼저" 만들고, 이후 Recipe의 명령어를 실행
    - Recipe: 빌드 대상을 생성하는 명령어. 반드시 들여쓰기 필요!
        - Recipe에서 Shell 명령어 실행 가능
        - Shell 명령어의 경우 터미널에 그 문장 자체가 출력되며, @를 붙이면 단순 명령어 실행만 한다
...
- 그러나.. 저렇게 작성하면 Incremental build를 위한 의존성 검사에서 헤더 파일 변경을 감지하지 못함. 
...
main.o : foo.h bar.h main.c
    ...
    bar.o : bar.h bar.c
```
- 이렇게 작성하면 헤더 파일만 변경되어도 의존성이 올바르게 탐지되며.. 레시피가 생략되어있지만 make가 자동으로 컴파일 수행
...
- 둘의 차이는 재귀로 인한 무한 루프에 빠지냐 그렇지 않냐인데.. 이건 좀 더 알아봐야겠다.. 뭔가 이유가 있으니까 구분한거겠지
